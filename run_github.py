...
def generate_html(alerts, regime, spy_rsi, history, charts, options_data, scan_summary=None, summary_history=None):
    """Generate the GitHub Pages dashboard.

    v5.7 UI PATCH:
    - preserves the GitHub static dashboard architecture
    - adds reliable table sorting
    - adds multi-select filters + search + reset
    - improves current-price fallback from prices.json/latest_scan prices
    - improves options details rendering and ticker-driven options open
    """
    ...
    prices_map = scan_summary.get("prices", {}) or {}

    def _add_price_fields(row):
        if not isinstance(row, dict):
            return row
        t = row.get("ticker")
        if t and not row.get("current_price") and t in prices_map:
            row["current_price"] = prices_map[t]
        ap = safe_float(row.get("alert_price", row.get("price", 0)), 0)
        cp = safe_float(row.get("current_price", 0), 0)
        if ap > 0 and cp > 0:
            row["pnl"] = round(cp - ap, 2)
            row["pnl_pct"] = round((cp - ap) / ap * 100, 2)
            row["change_pct"] = row["pnl_pct"]
        return row

    alerts_sorted = sorted([_add_price_fields(dict(a)) for a in alerts], key=lambda a:(a.get("rqg_score",0),a.get("confidence",0)), reverse=True)
    cutoff = (now - timedelta(days=30)).strftime("%Y-%m-%d")
    history_30 = [_add_price_fields(dict(h)) for h in history if h.get("date", "") >= cutoff]
    hist_wins=sum(1 for h in history_30 if h.get("result")=="WIN")
    hist_losses=sum(1 for h in history_30 if h.get("result")=="LOSS")
    ...
    data_json=json.dumps({"alerts":alerts_sorted,"history":history_30,"charts":charts,"options":options_data,"summary":scan_summary,"summaryHistory":summary_history[-30:],"prices":prices_map}, default=str).replace("</","<\\/")

    html = """<!DOCTYPE html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><meta http-equiv='refresh' content='900'><title>TopBottom v5.7 GitHub Interactive Dashboard __DATE__</title><script src='https://cdn.plot.ly/plotly-2.27.0.min.js'></script><style>
:root{--bg:#070b12;--panel:#111827;--panel2:#0b1220;--line:#243044;--text:#e5edf7;--muted:#8ba0b8;--cyan:#06b6d4;--green:#22c55e;--red:#ef4444;--yellow:#eab308;--blue:#60a5fa;--purple:#a855f7}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:Segoe UI,Arial,sans-serif;font-variant-numeric:tabular-nums}.top{position:sticky;top:0;background:#09111f;border-bottom:1px solid var(--line);padding:14px 22px;z-index:20}h1{margin:0;color:var(--cyan);font-size:20px}.sub{color:var(--muted);font-size:12px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;padding:18px 22px}.card,.box{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:15px}.label{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.04em}.value{font-size:28px;font-weight:800}.tabs{display:flex;gap:2px;padding:0 22px;background:#09111f;border-bottom:1px solid var(--line);flex-wrap:wrap;position:sticky;top:58px;z-index:15}.tab{padding:13px 18px;color:var(--muted);cursor:pointer;font-weight:700;border-bottom:3px solid transparent}.tab.active{color:var(--cyan);border-bottom-color:var(--cyan)}.page{display:none;padding:20px 22px}.page.active{display:block}.toolbar{display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin:0 0 12px 0}.filterBtn{background:#101827;color:var(--muted);border:1px solid var(--line);border-radius:999px;padding:7px 11px;cursor:pointer;font-weight:800;font-size:12px}.filterBtn.active{background:#063344;color:#67e8f9;border-color:#06b6d4}.resetBtn{background:#2a1220;color:#fecaca;border:1px solid #7f1d1d;border-radius:999px;padding:7px 11px;cursor:pointer;font-weight:800;font-size:12px}.search{min-width:250px}table{width:100%;border-collapse:collapse;background:var(--panel);border:1px solid var(--line);font-size:13px}th{background:var(--panel2);color:var(--muted);text-align:left;padding:10px;text-transform:uppercase;font-size:11px;position:sticky;top:112px;z-index:5;cursor:pointer;user-select:none}th:hover{color:var(--cyan)}th.sort-asc::after{content:' ▲';color:var(--cyan)}th.sort-desc::after{content:' ▼';color:var(--cyan)}td{padding:10px;border-top:1px solid #263247;vertical-align:top}.pill{padding:3px 9px;border-radius:99px;font-weight:800;font-size:11px;display:inline-block}.bull{background:#22c55e22;color:var(--green)}.bear{background:#ef444422;color:var(--red)}.neutral{background:#eab30822;color:var(--yellow)}.ticker{color:var(--cyan);font-weight:900}.pos{color:var(--green)}.neg{color:var(--red)}.chips span{display:inline-block;margin:2px;padding:3px 7px;border-radius:8px;background:#1e293b;color:#cbd5e1;font-size:11px}.chartbox{height:560px;background:var(--panel);border:1px solid var(--line);border-radius:12px;margin-top:12px}select,input{background:var(--panel2);color:var(--text);border:1px solid var(--line);border-radius:8px;padding:8px}.two{display:grid;grid-template-columns:repeat(auto-fit,minmax(360px,1fr));gap:14px}.reason{max-width:620px;white-space:normal}.small{font-size:12px;color:var(--muted)}.actionBtn{background:#0f2a3a;color:#67e8f9;border:1px solid #155e75;border-radius:8px;padding:5px 8px;cursor:pointer;font-weight:800}.warn{color:var(--yellow)}
</style></head><body><div class='top'><h1>TopBottom Universal v5.7 — GitHub Interactive Dashboard</h1><div class='sub'>Generated __SCAN_TIME__ • Regime __REGIME__ • SPY RSI __SPY_RSI__ • Sort + filters + current price + options patch</div></div>
...
<script>
const DATA=__DATA__;
const STATE={alerts:{q:'',signals:new Set(),sectors:new Set(),quality:new Set()},blocked:{q:'',signals:new Set(),sectors:new Set(),stages:new Set()},history:{q:'',signals:new Set(),results:new Set()},sort:{}};
function esc(x){return String(x??'').replace(/[&<>]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[m]))}
function qs(v){return String(v??'').toLowerCase()}
function show(id,el){document.querySelectorAll('.page').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));document.getElementById(id).classList.add('active');if(el)el.classList.add('active');if(id==='charts')drawChart();if(id==='options')renderOptions()}
function pct(v){let n=Number(v);return isFinite(n)?(n>=0?'+':'')+n.toFixed(2)+'%':'N/A'}
function money(v){let n=Number(v);return isFinite(n)&&n!==0?'$'+n.toFixed(2):'N/A'}
function cls(v){return v==='STRONG'||v==='BOTTOM'||v==='WIN'?'bull':v==='WEAK'||v==='TOP'||v==='LOSS'?'bear':'neutral'}
function pill(v){return '<span class="pill '+cls(v)+'">'+esc(v)+'</span>'}
function chips(obj){if(!obj)return '';if(Array.isArray(obj))return '<div class=chips>'+obj.map(x=>'<span>'+esc(x)+'</span>').join('')+'</div>';return '<div class=chips>'+Object.entries(obj).map(([k,v])=>'<span>'+esc(k)+': <b>'+esc(v)+'</b></span>').join('')+'</div>'}
function rowText(r){return qs(Object.values(r||{}).map(v=>typeof v==='object'?JSON.stringify(v):v).join(' '))}
function getPrice(r){let t=r.ticker, p=Number(r.current_price||0); if(!p && DATA.prices && t) p=Number(DATA.prices[t]||0); return p||0}
function pnlPct(r){let ap=Number(r.alert_price||r.price||0), cp=getPrice(r); return ap>0&&cp>0?((cp-ap)/ap*100):Number(r.pnl_pct||r.change_pct||0)}
function sortRows(key, rows, cols){let st=STATE.sort[key]||{}; if(st.idx===undefined)return rows; let col=cols[st.idx]; let dir=st.dir==='asc'?1:-1; return rows.slice().sort((a,b)=>{let av=col[2]?col[2](a):col[1](a), bv=col[2]?col[2](b):col[1](b); let an=Number(String(av).replace(/[$,%]/g,'')), bn=Number(String(bv).replace(/[$,%]/g,'')); if(isFinite(an)&&isFinite(bn)) return (an-bn)*dir; return String(av??'').localeCompare(String(bv??''))*dir})}
function tbl(key,cols,rows){if(!rows||!rows.length)return '<div class="box">No rows.</div>'; rows=sortRows(key,rows,cols); let st=STATE.sort[key]||{}; let h='<table><thead><tr>'+cols.map((c,i)=>`<th onclick="setSort('${key}',${i})" class="${st.idx===i?'sort-'+st.dir:''}">${c[0]}</th>`).join('')+'</tr></thead><tbody>'; for(const r of rows)h+='<tr>'+cols.map(c=>'<td>'+c[1](r)+'</td>').join('')+'</tr>'; return h+'</tbody></table>'}
function setSort(key,idx){let st=STATE.sort[key]||{}; STATE.sort[key]={idx:idx,dir:(st.idx===idx&&st.dir==='desc')?'asc':'desc'}; rerenderActive()}
function toggle(set,val){set.has(val)?set.delete(val):set.add(val)}
function filterBtn(page,type,val,label){let active=STATE[page][type].has(val);let v=JSON.stringify(String(val));return `<button class="filterBtn ${active?'active':''}" onclick='toggle(STATE.${page}.${type},${v});rerenderActive()'>${esc(label||val)}</button>`}
function toolbar(page, rows, config){let html=`<div class="toolbar"><input class="search" placeholder="Search ticker, sector, reason..." value="${esc(STATE[page].q)}" oninput="STATE.${page}.q=this.value;rerenderActive()">`; for(const group of config){let vals=[...new Set(rows.map(group.get).filter(Boolean))].sort(); for(const v of vals) html+=filterBtn(page,group.state,v,group.label?group.label(v):v)} html+=`<button class="resetBtn" onclick="resetFilters('${page}')">Reset</button></div>`; return html}
function resetFilters(page){STATE[page].q=''; for(const k of Object.keys(STATE[page])) if(STATE[page][k] instanceof Set) STATE[page][k].clear(); rerenderActive()}
function passSet(v,set){return !set||set.size===0||set.has(v)}
function openChart(tk){let cd=(DATA.charts||{})[tk]; if(!cd){alert('No chart data for '+tk+'. Chart data is generated only for selected tickers each scan.');return} let cs=document.getElementById('chartSel'); if(cs){cs.value=tk} drawChart(); show('charts',document.querySelectorAll('.tab')[4])}
function openOptions(tk){let os=document.getElementById('optSel'); if(os){ if(![...os.options].some(o=>o.value===tk)){let opt=document.createElement('option');opt.value=tk;opt.textContent=tk;os.appendChild(opt)} os.value=tk } renderOptions(tk); show('options',document.querySelectorAll('.tab')[5])}
function renderSummary(){let s=DATA.summary||{},h=DATA.summaryHistory||[];let wholeLeaders=(s.sectors||[]).slice().sort((a,b)=>Number(b.session_return||0)-Number(a.session_return||0)).slice(0,3);let wholeLag=(s.sectors||[]).slice().sort((a,b)=>Number(a.session_return||0)-Number(b.session_return||0)).slice(0,3);document.getElementById('summary').innerHTML=`<div class='box'><b>Current scan:</b> ${esc(s.scan_time)} | Regime <b>${esc(s.regime)}</b> | RSI <b>${esc(s.spy_rsi)}</b> | TimeQuality <b>${esc(s.time_quality)}</b> | Passed <b>${esc(s.filtered)}/${esc(s.stocks_scanned)}</b> | Alerts <b>${esc(s.alerts)}</b> | Blocked <b>${esc(s.blocked)}</b> | RQG <b>${s.rqg_gate?'ON':'OBS'}/${esc(s.rqg_threshold)}</b></div><div class='two'><div class='box'><h3>Recent Relative Strength</h3><b>Leaders:</b> ${(s.leaders_recent||[]).map(x=>esc(x.sector)+' '+pill(x.strength)+' '+pct(x.diff)).join(', ')||'N/A'}<br><b>Laggards:</b> ${(s.laggards_recent||[]).map(x=>esc(x.sector)+' '+pill(x.strength)+' '+pct(x.diff)).join(', ')||'N/A'}</div><div class='box'><h3>Whole-Day View</h3><b>Leaders:</b> ${wholeLeaders.map(x=>esc(x.sector)+' '+pct(x.session_return)).join(', ')||'N/A'}<br><b>Laggards:</b> ${wholeLag.map(x=>esc(x.sector)+' '+pct(x.session_return)).join(', ')||'N/A'}</div></div><div class='two'><div class='box'><h3>Pending by Stage</h3>${chips(s.pending_by_stage)}</div><div class='box'><h3>Pending by Sector</h3>${chips(s.pending_by_sector)}</div></div><h2>Last GitHub Run Comparison</h2>`+tbl('summary',[['Run',r=>esc(r.scan_time),r=>r.scan_time],['Regime',r=>pill(r.regime),r=>r.regime],['RSI',r=>esc(r.spy_rsi),r=>r.spy_rsi],['Filtered',r=>esc(r.filtered),r=>r.filtered],['Alerts',r=>esc(r.alerts),r=>r.alerts],['Blocked',r=>esc(r.blocked),r=>r.blocked],['Pending',r=>esc((r.candidates||[]).length||r.pending_total||0),r=>(r.candidates||[]).length||r.pending_total||0]],h.slice().reverse())}
function renderAlerts(){let rows=(DATA.alerts||[]).slice(); rows=rows.filter(r=>passSet(r.signal,STATE.alerts.signals)&&passSet(r.sector,STATE.alerts.sectors)&&passSet(r.rqg_label,STATE.alerts.quality)&&(!STATE.alerts.q||rowText(r).includes(qs(STATE.alerts.q)))); let cols=[['Ticker',r=>'<span class=ticker style="cursor:pointer" onclick="openChart(&quot;'+esc(r.ticker)+'&quot;)">'+esc(r.ticker)+'</span>',r=>r.ticker],['Signal',r=>pill(r.signal),r=>r.signal],['Alert$',r=>money(r.alert_price),r=>r.alert_price],['Current$',r=>getPrice(r)?money(getPrice(r)):'<span class=warn>N/A</span>',r=>getPrice(r)],['P&L%',r=>{let p=pnlPct(r);return '<span style="color:'+(p>0?'var(--green)':p<0?'var(--red)':'var(--muted)')+'">'+p.toFixed(2)+'%</span>'},r=>pnlPct(r)],['Conf',r=>esc(r.confidence),r=>r.confidence],['RQG',r=>'<b>'+esc(r.rqg_score)+'</b> '+esc(r.rqg_label),r=>r.rqg_score],['Options',r=>'<button class=actionBtn onclick="openOptions(&quot;'+esc(r.ticker)+'&quot;)">Gear</button>',r=>r.ticker],['MTF',r=>esc(r.mtf_status),r=>r.mtf_status],['Sector',r=>esc(r.sector)+' '+pill(r.sector_strength),r=>r.sector],['Reason Tags',r=>chips((r.rqg_reasons||[]).slice(0,10)),r=>(r.rqg_reasons||[]).join(' ')]]; document.getElementById('alerts').innerHTML=toolbar('alerts',DATA.alerts||[],[{state:'signals',get:r=>r.signal},{state:'sectors',get:r=>r.sector},{state:'quality',get:r=>r.rqg_label}])+'<div class=small>Showing '+rows.length+' of '+(DATA.alerts||[]).length+' alerts. Click any header to sort. Gear opens options.</div>'+tbl('alerts',cols,rows)}
function renderSectors(){let rows=(DATA.summary&&DATA.summary.sectors)||[]; document.getElementById('sectors').innerHTML=tbl('sectors',[['Sector',r=>'<span class=ticker>'+esc(r.sector)+'</span>',r=>r.sector],['State',r=>pill(r.strength),r=>r.strength],['Recent Rel',r=>pct(r.diff),r=>r.diff],['Sector Recent',r=>pct(r.sector_return),r=>r.sector_return],['SPY Recent',r=>pct(r.spy_return),r=>r.spy_return],['Whole Day',r=>pct(r.session_return),r=>r.session_return],['SPY Day',r=>pct(r.session_spy_return),r=>r.session_spy_return],['Close',r=>money(r.close),r=>r.close],['Data',r=>esc(r.source||'')+' age '+esc(r.age_min||'N/A')+'m<br><span class=small>'+esc(r.latest||'')+'</span>',r=>r.age_min],['Reason',r=>'<div class=reason>'+esc(r.reason||'')+'</div>',r=>r.reason]],rows)}
function renderBlocked(){let b=((DATA.summary||{}).blocked_alerts||[]).slice(); let c=((DATA.summary||{}).candidates||[]).slice(); b=b.filter(r=>passSet(r.signal,STATE.blocked.signals)&&passSet(r.sector,STATE.blocked.sectors)&&(!STATE.blocked.q||rowText(r).includes(qs(STATE.blocked.q)))); c=c.filter(r=>passSet(r.signal,STATE.blocked.signals)&&passSet(r.sector,STATE.blocked.sectors)&&passSet(r.stage,STATE.blocked.stages)&&(!STATE.blocked.q||rowText(r).includes(qs(STATE.blocked.q)))); let all=((DATA.summary||{}).blocked_alerts||[]).concat((DATA.summary||{}).candidates||[]); document.getElementById('blocked').innerHTML=toolbar('blocked',all,[{state:'signals',get:r=>r.signal},{state:'sectors',get:r=>r.sector},{state:'stages',get:r=>r.stage}])+'<h2>Blocked by RQG</h2>'+tbl('blocked1',[['Ticker',r=>'<span class=ticker onclick="openChart(&quot;'+esc(r.ticker)+'&quot;)">'+esc(r.ticker)+'</span>',r=>r.ticker],['Signal',r=>pill(r.signal),r=>r.signal],['Sector',r=>esc(r.sector),r=>r.sector],['Price',r=>money(r.price),r=>r.price],['RQG',r=>esc(r.rqg_score)+' '+esc(r.rqg_label),r=>r.rqg_score],['Reasons',r=>chips(r.rqg_reasons||[]),r=>(r.rqg_reasons||[]).join(' ')]],b)+'<h2>Waiting / Tracking Candidates</h2>'+tbl('blocked2',[['Ticker',r=>'<span class=ticker onclick="openChart(&quot;'+esc(r.ticker)+'&quot;)">'+esc(r.ticker)+'</span>',r=>r.ticker],['Signal',r=>pill(r.signal),r=>r.signal],['Sector',r=>esc(r.sector),r=>r.sector],['Stage',r=>esc(r.stage),r=>r.stage],['C5',r=>esc(r.c5),r=>r.c5],['C15',r=>esc(r.c15||''),r=>r.c15||0],['Price',r=>money(r.price),r=>r.price]],c)}
function renderHistory(){let h=(DATA.history||[]).slice().reverse(); h=h.filter(r=>passSet(r.signal,STATE.history.signals)&&passSet(r.result||'PENDING',STATE.history.results)&&(!STATE.history.q||rowText(r).includes(qs(STATE.history.q)))); document.getElementById('history').innerHTML=toolbar('history',DATA.history||[],[{state:'signals',get:r=>r.signal},{state:'results',get:r=>r.result||'PENDING'}])+'<div class=box><b>30-day rolling history:</b> '+h.length+' visible of '+(DATA.history||[]).length+' records.</div>'+tbl('history',[['Date',r=>esc(r.date),r=>r.date],['Time',r=>esc(r.time||r.alert_time||''),r=>r.time||r.alert_time||''],['Ticker',r=>'<span class=ticker onclick="openChart(&quot;'+esc(r.ticker)+'&quot;)">'+esc(r.ticker)+'</span>',r=>r.ticker],['Signal',r=>pill(r.signal),r=>r.signal],['Alert $',r=>money(r.alert_price),r=>r.alert_price],['Current $',r=>getPrice(r)?money(getPrice(r)):'<span class=warn>N/A</span>',r=>getPrice(r)],['Result',r=>pill(r.result||'PENDING'),r=>r.result||'PENDING'],['P&L %',r=>pct(pnlPct(r)),r=>pnlPct(r)],['RQG',r=>esc(r.rqg_score||''),r=>r.rqg_score||0],['Setup',r=>esc(r.setup_type||''),r=>r.setup_type||'']],h)}
function renderLogs(){let s=DATA.summary||{};document.getElementById('logs').innerHTML='<h2>Readable End-of-Scan Log</h2><div class=box><pre style="white-space:pre-wrap;line-height:1.45">'+esc(JSON.stringify({scan_time:s.scan_time,regime:s.regime,spy_rsi:s.spy_rsi,time_quality:s.time_quality,passed:String(s.filtered)+'/'+String(s.stocks_scanned),alerts:s.alerts,blocked:s.blocked,pending_total:s.pending_total,pending_by_stage:s.pending_by_stage,pending_by_sector:s.pending_by_sector,prices_count:Object.keys(DATA.prices||{}).length,sector_state:s.sectors,last_runs:DATA.summaryHistory},null,2))+'</pre></div>'}
function initSelectors(){let c=Object.keys(DATA.charts||{}), cs=document.getElementById('chartSel');cs.innerHTML=c.map(x=>'<option value="'+esc(x)+'">'+esc(x)+'</option>').join('');let o=Object.keys(DATA.options||{}), os=document.getElementById('optSel');os.innerHTML=o.map(x=>'<option value="'+esc(x)+'">'+esc(x)+'</option>').join('')}
function drawChart(){let t=document.getElementById('chartSel').value,d=(DATA.charts||{})[t];if(!d){document.getElementById('chart').innerHTML='<div class=box>No chart data</div>';return}let traces=[{x:d.timestamps,open:d.open,high:d.high,low:d.low,close:d.close,type:'candlestick',name:t},{x:d.timestamps,y:d.vwap,type:'scatter',name:'VWAP'},{x:d.timestamps,y:d.ema9,type:'scatter',name:'EMA9'},{x:d.timestamps,y:d.ema21,type:'scatter',name:'EMA21'}];Plotly.newPlot('chart',traces,{paper_bgcolor:'#111827',plot_bgcolor:'#111827',font:{color:'#e5edf7'},title:t+' intraday chart',xaxis:{rangeslider:{visible:false},gridcolor:'#243044'},yaxis:{side:'right',gridcolor:'#243044'}},{responsive:true})}
function renderOptions(forceTk){let t=forceTk||document.getElementById('optSel').value,o=(DATA.options||{})[t];if(!t){document.getElementById('optBox').innerHTML='<div class=box>No options ticker selected.</div>';return}if(!o){document.getElementById('optBox').innerHTML='<div class=box><b>'+esc(t)+'</b><br>No options data embedded for this ticker. GitHub scan currently fetches options only for the first few alerts to stay under Action time limits.</div>';return}if(o.error){document.getElementById('optBox').innerHTML='<div class=box><b>'+esc(t)+'</b><br>Options unavailable: '+esc(o.error)+'</div>';return}document.getElementById('optBox').innerHTML='<div class=box><b>'+esc(t)+'</b> Spot '+money(o.spot)+' Exp '+esc(o.expiration)+' DTE '+esc(o.dte)+' IV '+esc(o.avg_iv)+'% PCR '+esc(o.pcr)+'</div><h3>Suggested Strategies</h3>'+tbl('options1',[['Name',r=>esc(r.name),r=>r.name],['Cost',r=>esc(r.cost),r=>r.cost],['Max Profit',r=>esc(r.max_profit),r=>r.max_profit],['Breakeven',r=>esc(r.breakeven),r=>r.breakeven],['Legs',r=>(r.legs||[]).map(l=>esc(l.action+' '+l.type+' '+l.strike+' @ '+l.price)).join('<br>'),r=>(r.legs||[]).length]],o.strategies||[])+'<h3>Unusual Options</h3>'+tbl('options2',[['Type',r=>esc(r.type),r=>r.type],['Strike',r=>esc(r.strike),r=>r.strike],['Vol',r=>esc(r.volume),r=>r.volume],['OI',r=>esc(r.oi),r=>r.oi],['Ratio',r=>esc(r.ratio),r=>r.ratio]],o.unusual||[])}
function rerenderActive(){let id=document.querySelector('.page.active')?.id||'summary'; if(id==='summary')renderSummary(); if(id==='alerts')renderAlerts(); if(id==='sectors')renderSectors(); if(id==='blocked')renderBlocked(); if(id==='history')renderHistory(); if(id==='logs')renderLogs(); if(id==='options')renderOptions()}
renderSummary();renderAlerts();renderSectors();renderBlocked();renderHistory();renderLogs();initSelectors();
</script></body></html>"""
    repl = {
        "__DATE__": scan_date, "__SCAN_TIME__": scan_time, "__REGIME__": str(regime), "__SPY_RSI__": f"{float(spy_rsi):.1f}",
...
def main():
    ...
    # Make prices available inside the embedded dashboard JSON.
    scan_summary["prices"] = prices_data
