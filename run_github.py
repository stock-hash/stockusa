...
function renderAlerts(){document.getElementById('alerts').innerHTML=tbl([['Ticker',r=>'<span class=ticker style="cursor:pointer" onclick="openTk(\''+esc(r.ticker)+'\')">' +esc(r.ticker)+'</span>'],['Signal',r=>pill(r.signal)],['Price',r=>money(r.alert_price)],['Current$',r=>money(r.current_price)],['P&L%',r=>{let ap=Number(r.alert_price)||0,cp=Number(r.current_price)||0;let p=ap>0&&cp>0?((cp-ap)/ap*100):0;return '<span style="color:'+(p>0?'var(--green)':p<0?'var(--red)':'var(--muted)')+'">'+p.toFixed(2)+'%</span>'}],['Conf',r=>esc(r.confidence)],['RQG',r=>'<b>'+esc(r.rqg_score)+'</b> '+esc(r.rqg_label)],['Time',r=>'<span style="font-size:11px;color:var(--muted)">'+esc(r.time||'')+'</span>'],['MTF',r=>esc(r.mtf_status)],['Sector',r=>esc(r.sector)+' '+pill(r.sector_strength)],['Reason Tags',r=>chips((r.rqg_reasons||[]).slice(0,10))]],DATA.alerts)}
function renderSectors(){document.getElementById('sectors').innerHTML=tbl([['Sector',r=>'<span class=ticker>'+esc(r.sector)+'</span>'],['State',r=>pill(r.strength)],['Price',r=>money(r.close)],['Recent Rel',r=>pct(r.diff)],['Sector Recent',r=>pct(r.sector_return)],['SPY Recent',r=>pct(r.spy_return)],['Whole Day',r=>pct(r.session_return)],['SPY Day',r=>pct(r.session_spy_return)],['Close',r=>money(r.close)],['Data',r=>esc(r.source||'')+' age '+esc(r.age_min||'N/A')+'m<br><span class=small>'+esc(r.latest||'')+'</span>'],['Reason',r=>'<div class=reason>'+esc(r.reason||'')+'</div>']],DATA.summary.sectors)}
function renderBlocked(){document.getElementById('blocked').innerHTML='<h2>Blocked by RQG</h2>'+tbl([['Ticker',r=>'<span class=ticker style="cursor:pointer" onclick="openTk(\''+esc(r.ticker)+'\')">' +esc(r.ticker)+'</span>'],['Signal',r=>pill(r.signal)],['Sector',r=>esc(r.sector)],['Price',r=>money(r.price)],['RQG',r=>esc(r.rqg_score)+' '+esc(r.rqg_label)],['Reasons',r=>chips(r.rqg_reasons||[])]],DATA.summary.blocked_alerts)+'<h2>Waiting / Tracking Candidates</h2>'+tbl([['Ticker',r=>'<span class=ticker style="cursor:pointer" onclick="openTk(\''+esc(r.ticker)+'\')">' +esc(r.ticker)+'</span>'],['Signal',r=>pill(r.signal)],['Sector',r=>esc(r.sector)],['Stage',r=>esc(r.stage)],['C5',r=>esc(r.c5)],['C15',r=>esc(r.c15||'')],['Price',r=>money(r.price)]],DATA.summary.candidates)}
function renderHistory(){let h=(DATA.history||[]).slice().reverse();document.getElementById('history').innerHTML='<div class=box><b>30-day rolling history:</b> '+h.length+' records. This data is loaded from docs/scan_history.json and preserved across GitHub runs when the workflow commits docs updates.</div>'+tbl([['Date',r=>esc(r.date)],['Time',r=>esc(r.time||r.alert_time||'')],['Ticker',r=>'<span class=ticker style="cursor:pointer" onclick="openTk(\''+esc(r.ticker)+'\')">' +esc(r.ticker)+'</span>'],['Signal',r=>pill(r.signal)],['Alert $',r=>money(r.alert_price)],['Current $',r=>money(r.current_price)],['Result',r=>pill(r.result||'PENDING')],['P&L %',r=>pct(r.change_pct||r.pnl_pct)],['RQG',r=>esc(r.rqg_score||'')],['Setup',r=>esc(r.setup_type||'')]],h)}
...
function openTk(tk){let cd=DATA.charts[tk];if(!cd){alert('No chart data for '+tk+'. Charts are generated during market scans.');return}let traces=[{x:cd.timestamps,open:cd.open,high:cd.high,low:cd.low,close:cd.close,type:'candlestick',name:tk,increasing:{line:{color:'#22c55e'}},decreasing:{line:{color:'#ef4444'}}},{x:cd.timestamps,y:cd.vwap,type:'scatter',mode:'lines',name:'VWAP',line:{color:'#eab308',width:1.5,dash:'dot'}},{x:cd.timestamps,y:cd.ema9,type:'scatter',mode:'lines',name:'EMA9',line:{color:'#f97316',width:1}},{x:cd.timestamps,y:cd.ema21,type:'scatter',mode:'lines',name:'EMA21',line:{color:'#a855f7',width:1}}];if(cd.bb_upper)traces.push({x:cd.timestamps,y:cd.bb_upper,type:'scatter',mode:'lines',name:'BB+',line:{color:'rgba(96,165,250,0.3)',width:1}},{x:cd.timestamps,y:cd.bb_lower,type:'scatter',mode:'lines',name:'BB-',line:{color:'rgba(96,165,250,0.3)',width:1},fill:'tonexty',fillcolor:'rgba(96,165,250,0.04)'});Plotly.newPlot('chart',traces,{paper_bgcolor:'#111827',plot_bgcolor:'#111827',font:{color:'#e5edf7'},title:tk+' Intraday Chart (click ticker to view)',xaxis:{rangeslider:{visible:false},gridcolor:'#243044'},yaxis:{side:'right',gridcolor:'#243044'}},{responsive:true});show('charts',document.querySelectorAll('.tab')[4])}
